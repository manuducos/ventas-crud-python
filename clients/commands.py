...
@click.option('-p', '--position',
                type=str,
                prompt=True,
                help='Client\'s position: ')
@click.pass_context
def create(ctx, name, company, email, position):
    """
    Creates a new client -
    Use: ventas clients create
    """
...
def delete(ctx, client_uid):
    ...
    if client:
        clients_service.delete_client(Client(**client[0]).uid)
